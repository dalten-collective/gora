  ...
  ::
    ...
    ?~  nul.diff
      =?    offers.logs
          ...
          ::
              %.  our.bol
              ~(has in ~(key by stak.diff))
          ==
        (~(del in offers.logs) id.diff)
      =.  pita
        (~(put by pita) id.diff [%s +.diff])
      `state
      ::
    ?>  %+  levy  u.nul.diff
